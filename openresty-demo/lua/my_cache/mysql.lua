...
if not res then
    ngx.say("drop table err: ", err, ": ", errno, ": ", sqlstate, ".")
    return
end
...
res, err, errno, sqlstate = db:query("create table cats "
    ...
    .. "name varchar(5))")
if not res then
    ngx.say("create table err: ", err, ": ", errno, ": ", sqlstate, ".")
    return
end
...
res, err, errno, sqlstate = db:query("insert into cats (name) "
    .. "values (\'Bob\'),(\'\'),(null)")
if not res then
    ngx.say("insert err: ", err, ": ", errno, ": ", sqlstate, ".")
    return
end
...
if not res then
    ngx.say("1--bad result: ", err, ": ", errno, ": ", sqlstate, ".")
    return
end
...
if not res then
    ngx.say("2--bad result: ", err, ": ", errno, ": ", sqlstate, ".")
    return
end
